...
compute_fragment_size_wps <- function(bam, targets, tag = "", window_size = 120,
    ...
    assertthat::assert_that(is.numeric(window_size), is.numeric(step_size),
    ...
    regions <-  purrr::pmap_dfr(list(targets$chr, targets$start, targets$end), 
    	function(x, y, z) { 
    		data.frame(chr = x, start = seq(y - window_size + 1, z + window_size - 1, by = step_size),
    		    stringsAsFactors = F) %>%
    	        dplyr::mutate(end = .data$start + window_size)
    	    })
    
    frag_ends_gr <- GenomicRanges::GRanges(rep(reads$chr, 2),
    ...
    regions <- regions %>%
        dplyr::mutate(WPS = (n_within_overlaps - n_side_overlaps),
            WPS_adjusted = WPS / ifelse(n_overlaps == 0, 1, n_overlaps),
