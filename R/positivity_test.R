...
positivity_test <- function(depths, altReads, rate, substitutions = NULL, seed = 123, n_simulations = 1e+04) {
    set.seed(seed)
    
    assertthat::assert_that(assertthat::noNA(depths), 
    	assertthat::not_empty(depths), 
    	is.numeric(depths))
    
    assertthat::assert_that(assertthat::noNA(altReads), 
    	assertthat::not_empty(altReads), 
    	is.numeric(altReads))
    
    assertthat::assert_that(is.numeric(depths), 
    	all(depths %% 1 == 0))

    assertthat::assert_that(is.numeric(altReads),
        all(altReads %% 1 == 0))
